...
paintrock_spectra_df %>% 
  ...
  #ally() %>% 
  #plyr::rename(tree_count = n) %>%
  #inner_join(paintrock_spectra_df, by="taxon_code") %>% #dim
  group_by(taxon_code) %>% 
  dplyr::mutate(sample_size = n()) %>% 
  dplyr::mutate(taxon_code_wN = glue('{taxon_code} {"(n="} {sample_size})')) %>%
...
ggplot(paintrock_spectra_df_tall, aes(Wavelength, Median_Reflectance), scales = "fixed") +
